...
from dolfinx.mesh import (CellType, create_box, exterior_facet_indices, locate_entities_boundary,
                          locate_entities)
...
msh = create_box(MPI.COMM_WORLD, [np.array([0,0,0]), np.array([L, W, W])],
...
boundary_facets = locate_entities_boundary(msh, fdim, clamped_boundary)
...
f = fem.Constant(msh, ScalarType((0, 0, -rho*g))) 
a = ufl.inner(sigma(u), epsilon(v)) * ufl.dx            # k_form
L = ufl.dot(f, v) * ufl.dx + ufl.dot(T, v) * ds         # l_form ?
m_form = rho*ufl.dot(u, v)*ufl.dx                       # m_form

a_lhs = fem.form(a)
b_rhs = fem.form(m_form)

# https://fenicsproject.discourse.group/t/eigen-solver-syntax-in-dolfin-x/7369

A = fem.petsc.assemble_matrix(a_lhs, bcs=[bc])
A.assemble()
B = fem.petsc.assemble_matrix(b_rhs, bcs=[bc])
B.assemble()
